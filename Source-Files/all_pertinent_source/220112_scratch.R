...
extGene=function(gene="TP53",bigObj=hcv,titleTrim=50,textSize=12){
	require(tidyverse)
	require(ggpubr)
	require(janitor)
	...
	return(
		list(
			...
			dePlot=rbindlist(lapply(ints,function(Z) Z$de)) %>% mutate(list=(gsub("x_cond_|cond|cond_","",make_clean_names(list)))) %>% ggplot(aes(x=list,y=logFC,fill=-log10(adj.P.Val))) + geom_bar(stat='identity')+ facet_wrap(~study,nrow=1,scale="free_x") + scale_fill_viridis_c(option="H") + ggtitle(gene, "foldchange in basic comparisons") + theme_bw(base_size=textSize+4) + geom_hline(yintercept=0) + guides(x=guide_axis(angle=20))
		)
	)
}
